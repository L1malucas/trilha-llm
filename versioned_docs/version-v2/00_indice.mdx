...
- **Base larga** (módulos [1](01_matematica.md)–[7](07_transformers.mdx)): matemática, programação, ML clássico/moderno, DL, NLP clássico. Sem essa base, tudo o que vier depois é uso de caixa-preta.
- **Tronco profundo em LLMs** (módulos [8](08_llms_arquiteturas.md)–[15](15_engenharia_producao.mdx)): Transformers, arquiteturas, treinamento, alinhamento, eficiência, prompting, RAG, agentes, avaliação, produção.
...
## Como estudar: Clássico vs Acelerado

Esta trilha existe (ou vai existir) em duas versões, trocáveis pelo seletor de versão no topo do site — não são seções dentro da mesma página, são dois jeitos inteiros de percorrer o material, para dois perfis de estudo diferentes.
...
### Versão Acelerado (futura, ainda não publicada)

Pensada para quem já tem bagagem técnica e quer ir direto para implementação e projetos, buscando fundamentos teóricos só quando um projeto específico exigir — a ordem de navegação se inverte: começa pelos projetos práticos (mod. [20](20_projetos_integradores.md)) ou por uma implementação concreta, e volta aos módulos de base sob demanda, como referência pontual em vez de leitura sequencial obrigatória. Vai ser fortemente orientada a código (Python e TypeScript, seguindo a mesma convenção descrita mais abaixo) e vai referenciar a versão Clássico para explicações teóricas em vez de reexplicá-las — os dois materiais são complementares, não duplicados. Quando publicada, ela aparece como uma opção adicional no seletor de versão do site.
