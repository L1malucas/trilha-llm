...
> Versão **Acelerado**: prática primeiro, teoria sob demanda. Currículo de engenharia real, sem viés de fornecedor.
> Linguagens: **Python** (núcleo) e **TypeScript** (onde há ecossistema maduro).
> Saída: um arquivo `.md`/`.mdx` por módulo, este arquivo é o índice e o mapa.
...
## Prefere entender antes de construir?

Esta é a versão Acelerado. Se você prefere teoria completa antes de qualquer prática — intuição, exemplo resolvido, aplicação real, checkpoint, em cada módulo, na ordem 01→20 — a versão **Clássico** desta trilha cobre exatamente o mesmo território, na ordem inversa. Troque de versão pelo seletor no topo do site, ou acesse diretamente a [raiz da versão Clássico](/trilha-llm/v2/).
